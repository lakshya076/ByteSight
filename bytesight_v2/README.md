...
```bash
# Convert Microsoft Malware Challenge (.bytes) to PNG
python bytesight_v2/process_microsoft_challenge.py --cores 12

# Generate Windows Benign Dataset (from System32)
...
```bash
# Stage 1: Detect Malware vs Benign
python v2_main.py train --mode binary --epochs 15

# Stage 2: Identify Malware Family (Malware-only)
